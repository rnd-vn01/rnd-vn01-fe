...
export const SearchResults: React.FC<IQuickSearchResults> = ({ query }) => {
  ...
  return (
    <div
      ...
      className={`search-results w-full 
      ${!isLoading && Object.keys(results).length > 0 ? "search-results--showing" : ""}`}
      ...
      {!isLoading && Object.keys(results).length > 0 &&
        Object.keys(results).map((category: any, index: number) => {
          if (results[category].length > 0) {
            return (
              <div
                className={`search-results__block`}
                key={`quick-search-category-${index}`}
              >
                <div className="search-results__block--category">
                  {capitalize(category)}
                </div>
